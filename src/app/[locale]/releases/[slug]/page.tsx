...
async function ReleaseHero({ release }: { release: ReleasePageData }) {
        ...
        <div className="min-w-0 text-center md:text-left">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2 md:justify-start">
            ...
            <span className="rounded-full border border-[#CE1126]/15 bg-[#CE1126]/5 px-3 py-1 text-xs font-medium uppercase tracking-[0.16em] text-[#8B0000]">
              {t("artist.tracksCount", { count: release.tracks.length })}
            </span>
          </div>
        </div>
      </div>
...
async function ReleaseFactsCard({ release }: { release: ReleasePageData }) {
  ...
  const facts = [
    [t("releases.facts.type"), formatReleaseType(release.type)],
    [t("releases.facts.release"), formatDate(release)],
    [t("releases.facts.label"), release.label],
    [t("releases.facts.country"), release.country],
    [t("releases.facts.catalog"), release.catalogNumber],
    [t("releases.facts.barcode"), release.barcode],
      ...
      <PageSection className="mt-4">
        <div className="mx-auto max-w-6xl space-y-5">
          ...
          {moreFromArtist.length > 0 && (
            <ReleaseSection
              title={t("releases.moreFromArtist")}
          ...
          {moreFromDecade.length > 0 && releaseYear && (
            <ReleaseSection
              title={t("releases.moreFromDecade", { decade: `${Math.floor(releaseYear / 10) * 10}s` })}
          ...
          {dominantGenreResult.genre && dominantGenreResult.releases.length > 0 && (
            <ReleaseSection
              title={t("releases.moreInGenre", { genre: dominantGenreResult.genre.name })}
