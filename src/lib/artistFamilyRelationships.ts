...
/** Any Supabase client: the public anon one, or a privileged server-side one. */
...
/**
 * Presentation keys are what the reader sees, and they are gendered wherever the
 * language needs it. English has one word for a cousin, a nephew or niece and an
 * uncle or aunt in some positions; Spanish does not, so male_cousin/female_cousin
 * and nibling/uncle_aunt exist purely so the Spanish label can be primo, prima,
 * sobrino/a and tío/a. They are never editor choices: the editor picks the
 * relationship and the gender of the related artist decides the wording.
 */
export type FamilyPresentationKey = FamilyEditorLabel | "late_husband" | "late_wife" | "great_uncle_aunt" | "great_nibling" | "male_cousin" | "female_cousin" | "uncle_aunt" | "nibling";
...
const familyPresentationOrder: Record<FamilyPresentationKey, number> = {
  ...
  uncle: 40, aunt: 40, uncle_aunt: 40,
  ...
  cousin: 70, male_cousin: 70, female_cousin: 70,
  son: 80, daughter: 80, child: 80,
  nephew: 90, niece: 90, nibling: 90,
...
export function resolveFamilyRelationship(row: FamilyRelationshipRow, perspectiveId: string): FamilyRelationshipItem | null {
  ...
  else if (row.relationship_type === "uncle_aunt") labelKey = fromCanonical ? gendered(relative,"nephew","niece","nibling") : gendered(relative,"uncle","aunt","uncle_aunt");
  ...
  else if (row.relationship_type === "cousin") labelKey = gendered(relative,"male_cousin","female_cousin","cousin");
...
/**
 * Reads the family relationships of an artist.
 *
 * `artist_family_relationships` is world-readable but `artists` is restricted to
 * published rows, so the embedded artist/related_artist come back null whenever
 * either side is a draft, and resolveFamilyRelationship then drops the row. On
 * the public profile that is the behaviour we want: an unpublished artist must
 * not surface through someone else's family list.
 *
 * Admin callers are authorized to see drafts and must pass a service-role client
 * instead, or the manager silently shows nothing for every draft artist.
 */
export async function getArtistFamilyRelationships(artistId: string, client?: SupabaseLikeClient): Promise<FamilyRelationshipItem[]> {
  const db = client ?? getSupabaseClient();
  const { data, error } = await db.from("artist_family_relationships").select(SELECT).or(`artist_id.eq.${artistId},related_artist_id.eq.${artistId}`).order("created_at");
