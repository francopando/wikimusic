...
-- Reverts 20260905342600_rewrite_rokabanda_biography.sql.
--
...
El Bacano, El Ñoñito and Los Hombres Maduros are the titles that outlived the band. They are built the way Méndez built everything — a chorus that a crowd can sing back after one hearing, a comic character sketched in two lines, and a horn section that never lets the tempo sag.
...
El Bacano, El Ñoñito y Los Hombres Maduros son los títulos que sobrevivieron a la banda. Están armados como Méndez armaba todo: un coro que la gente devuelve cantando después de oírlo una vez, un personaje cómico dibujado en dos líneas, y una sección de metales que no deja caer el tiempo.
...
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rokabanda') AND locale = 'en'), 'c5fe71ff-e125-4289-b4fd-41a39ec23b9d', 'artist', '001831dd-3baa-4512-88f5-f420ec7c2619');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rokabanda') AND locale = 'en'), 'cf35097b-febc-4bef-9f03-37e302a4e362', 'artist', 'c73737c2-0106-4a87-8dbe-5f1650d34342');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rokabanda') AND locale = 'es'), '63b3efb9-cdcc-4ea4-b33e-f948bb4cb3f4', 'artist', '001831dd-3baa-4512-88f5-f420ec7c2619');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rokabanda') AND locale = 'es'), 'dc37aced-b7e8-43b3-bc75-77d920e99719', 'artist', 'c73737c2-0106-4a87-8dbe-5f1650d34342');
