...
--
-- THE LEGAL NAME IS SETTLED, AND BY HIM. The first version of this entry left
-- it open: the row stored "Caonabo Enrique Mesa Ureña" and several outlets
-- write "Luis Enrique Caonabo Mesa". The editor pointed at the cover of his own
-- book, which carries "TOXIC CROW / CAONABO ENRIQUE MESA UREÑA" under the
-- title. That is the artist's own attribution on a work he published, it
-- carries a Dominican ISBN (9789945922929), and Diario Libre used the same form
-- when it covered the launch. The stored value is correct and the question is
-- closed.
--
-- ADDED FOR THE SAME REASON: occupations now include writer.
--
...
-- between 2003 and 2004. Diario Libre, 5 August 2020, on the launch of
-- the book; Goodreads, Amazon, Audible and Apple Books for its date, length,
-- ISBN and audiobook edition.
--
...
UPDATE artists SET
       ...
       occupations = '["composer","producer","writer"]'::jsonb,
       ...
       instagram = 'toxiccrow',
       disambiguation = 'Rapper from Los Mina, founder of Complot Records and author of Cómo Hacerse Millonario Pensando',
...
In August 2020 he published Cómo Hacerse Millonario Pensando, eighty-three pages under his own legal name, with a Dominican ISBN and an audiobook edition. Diario Libre covered the launch.

A Dominican street rapper writing a book about how to think your way to money is not a departure from the records; it is the same argument in another format. What is unusual is that he registered it properly, which is the same instinct that produced the label.
...
En agosto de 2020 publicó Cómo Hacerse Millonario Pensando, ochenta y tres páginas bajo su propio nombre legal, con ISBN dominicano y edición en audiolibro. Diario Libre cubrió la puesta en circulación.

Que un rapero de calle dominicano escriba un libro sobre cómo pensar para llegar al dinero no es una desviación de los discos: es el mismo argumento en otro formato. Lo raro es que lo registró como se debe, que es el mismo instinto del que salió el sello.
...
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'toxic-crow') AND locale = 'en'), '539a04d1-eeb4-4125-9d1d-336d78000b2a', 'artist', '86fdf7d2-f8c3-457f-a318-20bb7b5a207e');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'toxic-crow') AND locale = 'en'), '61326a06-0e28-4c0a-becc-ccc1d1e1156a', 'artist', 'ec8ba439-3772-49ff-a218-05f5dc615763');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'toxic-crow') AND locale = 'en'), 'ea6826bf-cf38-47af-9e11-80740ceaafbd', 'artist', '102e7b78-ff98-4adc-9a54-ae73791fb176');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'toxic-crow') AND locale = 'es'), '23eea0c3-5520-4032-8ade-9e4a04f305f6', 'artist', 'ec8ba439-3772-49ff-a218-05f5dc615763');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'toxic-crow') AND locale = 'es'), 'a8b33adf-c26e-47c1-9fdb-530a81aad768', 'artist', '86fdf7d2-f8c3-457f-a318-20bb7b5a207e');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'toxic-crow') AND locale = 'es'), 'd089e48c-8281-4f41-a669-c39d1720441a', 'artist', '102e7b78-ff98-4adc-9a54-ae73791fb176');
