...
-- Discogs for the dates and the group names. A separate Spanish-language
-- account credited to merenguemaniard.com and recopied across Dominican music
-- pages, which independently gives the 4 April 1939 birth in Sabana de la Mar
-- and says he made his first name giving serenades in Santo Domingo -- the same
-- fact Ansonia records as serenading classmates, arriving by another route.
-- Instagram and Facebook archival accounts for Espíritu Burlón with Rafael
-- Solano's orchestra, 1976.
--
-- A PASSAGE REWRITTEN AFTER THE FACT, AND WHY. The first version of this entry
-- listed his student jobs as "buying and selling empty bottles in the morning,
-- selling roasted sweet peanuts in the evening, shining shoes at weekends".
-- Ansonia's own sentence reads "buying and selling empty bottles in the
-- morning, selling roasted sweet peanuts in the evening, and polishing shoes on
-- the weekends". That is one word of difference across twenty, which is not
-- paraphrase, it is transcription with a synonym dropped in. It has been
-- rewritten so the facts survive and the sentence is this catalogue's own. The
-- Ansonia page is signed editorial writing by Pablo E. Yglesias and is
-- copyrighted; the facts in it are not, but his sentences are.
--
...
He was born poor, and he paid for his own secondary schooling in the capital by working the street around his classes: bottles, peanuts, shoes, depending on the hour and the day. The voice came out of that work rather than alongside it. A vendor sells by singing, and the pregón is a form with its own rules about where a phrase has to land.
...
Nació pobre, y se pagó él mismo la secundaria en la capital trabajando la calle alrededor de las clases: botellas, maní, zapatos, según la hora y el día. La voz salió de ese trabajo y no al margen de él. Un vendedor vende cantando, y el pregón es una forma con reglas propias sobre dónde tiene que caer la frase.
...
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rico-lopez') AND locale = 'en'), '30308be6-01f0-4657-abe0-d1e907b9b89f', 'artist', 'f625be23-cfa4-43fe-8bb5-0879b2fa492f');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rico-lopez') AND locale = 'en'), '71c8a382-42ab-46ac-84ec-b59ecc21a67b', 'artist', 'ba42e200-51b0-437b-99ac-1daf39ade337');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rico-lopez') AND locale = 'en'), 'c0c3d487-de42-45d1-ab9a-efa413a2b646', 'artist', 'f625be23-cfa4-43fe-8bb5-0879b2fa492f');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rico-lopez') AND locale = 'en'), 'fa9b9e77-45ab-4843-85de-e1449ae88d2d', 'artist', '3f8bafec-e5ee-415d-8405-9551cceeeb9b');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rico-lopez') AND locale = 'es'), '304cd55d-918d-4d0c-950b-7c05ffa72883', 'artist', 'f625be23-cfa4-43fe-8bb5-0879b2fa492f');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rico-lopez') AND locale = 'es'), '504b1c74-e897-4004-a98d-6e3754fa5243', 'artist', '3f8bafec-e5ee-415d-8405-9551cceeeb9b');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rico-lopez') AND locale = 'es'), '6f9e0915-d512-4362-b9cd-f35fe7ecd117', 'artist', 'f625be23-cfa4-43fe-8bb5-0879b2fa492f');

INSERT INTO editorial_entity_references
  (editorial_document_id, occurrence_id, entity_type, target_artist_id)
VALUES ((SELECT id FROM editorial_documents WHERE owner_artist_id = (SELECT id FROM artists WHERE slug = 'rico-lopez') AND locale = 'es'), 'aee6c2e7-7f7d-45a7-9a88-32f60ef14446', 'artist', 'ba42e200-51b0-437b-99ac-1daf39ade337');
